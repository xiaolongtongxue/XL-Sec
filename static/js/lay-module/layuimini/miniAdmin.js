        ...
        render: function (options) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.login == false) {
                        layer.msg('登陆失效，请重新登录', { icon: 2 }); setTimeout(function () { window.location.href = "/admin/login.html"; }, 1000); return;
                    }
                    if (data == null) {
            ...
            /*$.getJSON(options.iniUrl,function(data){if(data.login==false){layer.msg('登陆已失效',{icon:2});setTimeout(function(){window.location.href="/admin/login.html"},1000);return}if(data==null){miniAdmin.error('暂无菜单信息')}else{miniAdmin.renderLogo(data.logoInfo);miniAdmin.renderClear(options.clearUrl);miniAdmin.renderAnim(options.pageAnim);miniAdmin.listen({homeInfo:data.homeInfo,multiModule:options.multiModule,});miniMenu.render({menuList:data.menuInfo,multiModule:options.multiModule,menuChildOpen:options.menuChildOpen});miniPage.render({homeInfo:data.homeInfo,menuList:data.menuInfo,multiModule:options.multiModule,renderPageVersion:options.renderPageVersion,menuChildOpen:options.menuChildOpen,listenSwichCallback:function(){miniAdmin.renderDevice()}});miniTheme.render({bgColorDefault:options.bgColorDefault,listen:true,});miniAdmin.deleteLoader(options.loadingTime)}}).fail(function(){miniAdmin.error('菜单接口有误')});*/
        },
